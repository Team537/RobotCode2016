...
import org.team537.robot.commands.drive.DriveArcade;
import org.team537.robot.commands.drive.DriveTank;
...
public class Drive extends Subsystem {
	...
	protected void initDefaultCommand() {
		setDefaultCommand(RobotMap.Driver.ARCADE_DRIVE ? new DriveArcade() : new DriveTank());
	}
	 ...
	 */
	public void driveSpeed(final double left, final double right) {
		changeControlMode(CANTalon.TalonControlMode.PercentVbus);
		leftDriveMaster.set(Maths.clamp(left, -1.0f, 1.0f));
		rightDriveMaster.set(-Maths.clamp(right, -1.0f, 1.0f));
	}
	...
	public void changeControlMode(final CANTalon.TalonControlMode mode) {
		...
		if (mode.equals(CANTalon.TalonControlMode.PercentVbus)) {
			...
			leftDriveMaster.setPID(0.085, 0.0, 0.0);
			leftDriveMaster.setF(0.0);
			// leftDriveMaster->SetClosedLoopOutputDirection(true);
			leftDriveMaster.reverseSensor(true); // SetSensorDirection
			leftDriveMaster.setCloseLoopRampRate(100);

			rightDriveMaster.setPID(0.085, 0.0, 0.0);
			rightDriveMaster.setF(0.0);
			// rightDriveMaster->SetClosedLoopOutputDirection(false);
			rightDriveMaster.reverseSensor(false); // SetSensorDirection
