...
void DriveTrain::Update(bool teleop)
{
	switch (state)
	{
		...
		case (DriveState::CONTROL_TEL):
			leftSpeedCurrent = joystick->GetRawAxis(1);
	...
	rightDrive2->Set(-rightSpeedCurrent);
	rightDrive3->Set(-rightSpeedCurrent);
	leftDrive4->Set(leftSpeedCurrent);
	leftDrive5->Set(leftSpeedCurrent);
	leftDrive6->Set(leftSpeedCurrent);
}
...
{
	state = newState;
	ChangeTalon(rightDrive1);
	ChangeTalon(rightDrive2);
	ChangeTalon(rightDrive3);
	ChangeTalon(leftDrive4);
	ChangeTalon(leftDrive5);
	ChangeTalon(leftDrive6);
}

void DriveTrain::ChangeTalon(CANTalon *talon)
{
	switch(state)
	{
		case(DriveState::SHOOTER_TEL):
